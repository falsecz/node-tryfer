    ...
    traces.forEach(function(tuple) {
      ...
      annotations.forEach(function(annotation) {
        ...
        if (annotation.name === zipkinCore_types.CLIENT_RECV ||
          ...
          if (!has(completedTraces, trace.traceId)) {
            completedTraces[trace.traceId] = {}
          }
          completedTraces[trace.traceId][trace.spanId] = 1;
          //self.annotationsForTrace[trace.traceId][trace.spanId] = [];
          //delete self.annotationsForTrace[trace.traceId][trace.spanId];
      ...
      if (has(self.annotationsForTrace, trace.traceId) && has(completedTraces, trace.traceId) && has(completedTraces[trace.traceId], trace.spanId)) {
        delete self.annotationsForTrace[trace.traceId][trace.spanId];
        delete completedTraces[trace.traceId][trace.spanId];
        if (Object.keys (self.annotationsForTrace[trace.traceId]).length == 0) {
          delete self.annotationsForTrace[trace.traceId];
        }
        if (Object.keys (completedTraces[trace.traceId]).length == 0) {
